...
for db in (ROOT / "bot_stats.db", ROOT / "vk_tokens.db"):
    if db.exists():
        try:
            con = sqlite3.connect(str(db)); integrity = con.execute("PRAGMA integrity_check").fetchone()[0]; con.close()
            report(integrity == "ok", f"SQLite integrity {db.name}", integrity)
        except Exception as exc: report(False, f"SQLite integrity {db.name}", str(exc))
redis_url = os.getenv("REDIS_URL", "").strip()
if redis_url:
    try:
        import redis as redis_pkg  # type: ignore
        report(True, "Redis client package", getattr(redis_pkg, "__version__", "installed"))
    except Exception as exc: report(False, "Redis client package", str(exc))
else:
    print("INFO  Redis — disabled (in-memory mode)")
report(bool(shutil.which("deno") or shutil.which("node")), "YouTube JS runtime", "deno/node available")
