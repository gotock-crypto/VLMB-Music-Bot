...
[[ -x "$APP_DIR/venv/bin/python3" ]] || { echo "Missing production venv — refusing rollback." >&2; exit 1; }
...
rsync -a --delete --exclude='.env' --exclude='venv/' --exclude='bot_stats.db' --exclude='bot_stats.db-shm' --exclude='bot_stats.db-wal' --exclude='vk_tokens.db' --exclude='bot.log' --exclude='bot-debug.log' "$TMP_DIR/MusBot/" "$APP_DIR/"
